...
public class BTree<T extends Comparable<T>> {
    ...
    public boolean insert(T value) {
        if (root == null) {
            ...
            while (node != null) {
                ...
                else {// start navigate to the leaf we want to insert to
                    if (value.compareTo(node.getKey(node.numberOfKeys() - 1)) > 0)
    ...
    private T deleteRec(T value, Node<T> toDel) {
        if (toDel == null)
            return null;
        Node<T> node = toDel;
        Node<T> theNode = getNode(value);
        while (!(node.equals(theNode))) {
            if (node.keysSize == minKeySize & node.parent != null) { // chek if node needs "combined" & not root.
                this.combined(node);
            }
            else { // start navigate to the node with the value to delete
                if (value.compareTo(node.getKey(node.numberOfKeys() - 1)) > 0) {
                    node = node.getChild(node.numberOfKeys());
                }
                else {
        ...
        if (node.keysSize == minKeySize & node.parent != null)
            this.combined(node);
        // fix everything until the node with the value to delete.
        if (node.numberOfChildren() == 0) { //case 1-node is a leaf.
            node.removeKey(value);
        }
        else { // internal node
            if (node.getChild(node.indexOf(value)).numberOfKeys() > minKeySize) {//case 2-replace value with pred.
                ...
                }//fix everything until the node with pred value.
                T pred = node.getKey(node.numberOfKeys() - 1);
                theNode.keys[theNode.indexOf(value)] = pred;
                node.removeKey((T) pred);
            }
            else if (node.getChild(node.indexOf(value) + 1).numberOfKeys() > minKeySize) {//case 3-replace value with succ.
                ...
                T succ = node.getKey(0);
                theNode.keys[theNode.indexOf(value)] = succ;
                node.removeKey((T) succ);
            }
            else { //case 4-merge with children.
                Node<T> leftChild = node.getChild(node.indexOf(value));
                Node<T> rightChild = node.getChild(node.indexOf(value) + 1);
                node.removeKey(value);
                node.removeChild(leftChild);
                rightChild.addKey(value);
    ...
    public boolean insert2pass(T value) {
        if (root == null) {
            ...
            double k = Math.log(size);
            double d = Math.log(minKeySize + 1);
            Node<T>[] path = new Node[((int) (k / d)) + 2];// array size equals to max hight of a Btree.
            Node<T> node = root;
            Integer start = 0;
            Integer end = 0;
            if (node.numberOfKeys() == maxKeySize) {// check if root needs "split".
                path[end] = node;
                end = end + 1;
            }
            else {
            ...
            while (node.numberOfChildren() != 0) {
                ...
                if (node.numberOfKeys() == maxKeySize) {// check if node needs "split".
                    path[end] = node;
                    end = end + 1;
                }
                else {
            ...
            if (path[start] == null) // the leaf contains less than 2t-1 keys.
                node.addKey(value);
            else {
                T middle=node.getKey(node.keysSize/2);
                while (start < end) {
                    split(path[start]);
                    start = start + 1;
                }
                int index=node.parent.indexOf(middle);
                if (value.compareTo(middle)<0) {
                    node.parent.getChild(index).addKey(value);
                }
                else {
                    node.parent.getChild(index+1).addKey(value);
                }
            }
        }
        size = size + 1;
