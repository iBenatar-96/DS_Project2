...
public class CuckooHashing {
    ...
    private int check;
    private Stack<Node> backtrack; //My adding.
    ...
    public CuckooHashing(HashMethods hf, int size) {
        ...
        backtrack = new Stack<Node>(); //my addition.
        makeEmpty();
        hashFunctions = hf;
    ...
    private boolean insertHelper1(String x) {
        Node toAdd = new Node(-2,-2, x); //creating new node to put in the stack.
        while (true) {
            int pos = -1;
            int kick_pos = -1;
            ...
            for (int count = 0; count <= MAXTRIES; count++) {
                for (int i = 0; i < numHashFunctions; i++) {
                    ...
                    if (array[pos] == null) {
                        array[pos] = x;
                        toAdd.currLocation = pos; //changing current location.
                        backtrack.push(toAdd); //adding node to stack.
                        currentSize++;
                        return true;
                    }
                } //inner loop closes.
                ...
                array[kick_pos] = x;
                x = tmp;
                toAdd.currLocation = kick_pos;
                backtrack.push(toAdd); //pushing node into the stack.
                toAdd = new Node(kick_pos, -2, x);
            }
            //insertion got into a cycle use overflow list
            this.stash.add(x);
            toAdd.currLocation = -1;
            backtrack.push(toAdd); //pushing node into the stack.
            return true;
        }
    }
	...
	public void undo() { //the main idea here is to keep nodes in the stack, that have fields of there current location and previous location.
        check = 1; //this is for not deleting the array.
        if (!(backtrack.isEmpty())){
            Node back = backtrack.pop();

            if(back.cameFrom == -2 & back.currLocation != -1) {
                array[back.currLocation] = null;
                return;
            }
            else if(back.currLocation == -1) {
                stash.remove(back.myName);
                if(back.cameFrom == -2)
                return;
            }
                ...
                array[back.currLocation] = null;
                array[back.cameFrom] = back.myName;
                back = backtrack.pop();
            }

            while(back.cameFrom!=-2){
                array[back.cameFrom] = back.myName; //backtracing previous locations of each element.
                back = backtrack.pop();
            }
            }
        check = 0;
	...
	public static class Node { //created new class
        private int cameFrom; //field that holds where he came from.
        private int currLocation; //field that hold new location.
        private String myName; //x.

        public Node(int cameFrom, int currLocation, String myName){
